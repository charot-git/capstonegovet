...
public class addPet3 extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private void CheckerGetter() {
        ...
        reference.child(uid).child("Pet").child("Pet3").setValue(hashMap).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void unused) {
                Toast.makeText(addPet3.this, petname +" has been added to your pets", Toast.LENGTH_SHORT).show();
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(addPet3.this, "Pet not added", Toast.LENGTH_SHORT).show();
            }
        });
    }
